...
public class Network extends Thread {
    ...
    private static Semaphore emptyBufferInputSemaphore;
    private static Semaphore emptyBufferOutputSemaphore;
    private static Semaphore fullBufferInputSemaphore;
    private static Semaphore fullBufferOutputSemaphore;
    private static Semaphore mutexInputSemaphore;
    private static Semaphore mutexOutputSemaphore;
    ...
    Network() {
        ...
        emptyBufferInputSemaphore = new Semaphore(maxNbPackets);
        emptyBufferOutputSemaphore = new Semaphore(maxNbPackets);
        fullBufferInputSemaphore = new Semaphore(0);
        fullBufferOutputSemaphore = new Semaphore(0);
        mutexInputSemaphore = new Semaphore(1);
        mutexOutputSemaphore = new Semaphore(1);
    }
    ...
    public static boolean send(Transactions inPacket) {
        try {
            emptyBufferInputSemaphore.acquire();
            mutexInputSemaphore.acquire();
...
//                System.out.println("\n DEBUG : Network.send() - inComingBuffer status " + getInBufferStatus());
            ...
            mutexInputSemaphore.release();
            fullBufferInputSemaphore.release();
        } catch (Exception e) {
            System.out.println("\n Interrupted.");
        }
        return true;
    ...
    public static boolean receive(Transactions outPacket) {
        try {
            fullBufferOutputSemaphore.acquire();
            mutexOutputSemaphore.acquire();
...
//                System.out.println("\n DEBUG : Network.receive() - outGoingBuffer status " + getOutBufferStatus());
            ...
            mutexOutputSemaphore.release();
            emptyBufferOutputSemaphore.release();
        } catch (Exception e) {
            System.out.println("\n Interrupted.");
        }
        return true;
    ...
    public static boolean transferOut(Transactions outPacket) {
        try {
            emptyBufferOutputSemaphore.acquire();
            mutexOutputSemaphore.acquire();
...
//                System.out.println("\n DEBUG : Network.transferOut() - outGoingBuffer status " + getOutBufferStatus());
            ...
            mutexOutputSemaphore.release();
            fullBufferOutputSemaphore.release();
    ...
    public static boolean transferIn(Transactions inPacket) {
        try {
            fullBufferInputSemaphore.acquire();
            mutexInputSemaphore.acquire();
...
//                System.out.println("\n DEBUG : Network.transferIn() - inComingBuffer status " + getInBufferStatus());
            ...
            mutexInputSemaphore.release();
            emptyBufferInputSemaphore.release();
        } catch (Exception e) {
            System.out.println("\n Interrupted.");
        }
        return true;
